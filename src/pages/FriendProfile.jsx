...
export default function FriendProfile() {
  ...
  if (!data) return <div className="p-12 text-center text-slate-500">User not found</div>;
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        <button 
          onClick={() => navigate(-1)}
          className="p-2 rounded-lg bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors"
        >
          <ArrowLeft size={20} />
        </button>
...
function FriendStatCard({ label, value, colorClass }) {
  return (
    <div className="glass-card p-5">
      <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 tracking-wider block mb-2">{label}</span>
